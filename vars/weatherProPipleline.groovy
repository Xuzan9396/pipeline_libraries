            ...
            stage('Build 程序') {
                ...
                steps {
                    script {
                        ...
                        // 设置环境变量和执行构建命令
                        def currentTime = "${new java.text.SimpleDateFormat('yyyy-MM-dd HH:mm:ss').format(new Date())}"
                        sh """
                        ...
                        CGO_ENABLED=0 GOOS=linux GOARCH=amd64 /home/ec2-user/data/go/bin/go build -x -ldflags "-X 'main.version=${env.VERSION}' -X 'main.buildTime=${currentTime}' -s -w" -o ./weather_api ./main.go
                        """
